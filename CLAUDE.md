...
This project uses a team of 5 specialized subagents defined in `.claude/agents/`. Each owns a domain and maintains its own knowledge base in `.claude/knowledge/`.
...
Agents can delegate to each other. A typical flow: `product-manager` drafts spec → `designer` adds flows → `engineer` implements → `devops` deploys → `marketer` announces.
...
- **Canonical sources live in `.claude/sources/`.** Pitch decks, landing drafts, and other reference material are stored as `.md` there so any agent can grep and read cheaply. Prefer reading from `.claude/sources/` over the original PDFs/PPTX.
...
- **Read-only by default for anything outside your domain.** Engineer touches code, Shipper touches deploy config, etc. When in doubt, propose and ask.
