...
- **Regression + test registry.** A new feature must never break a shipped one. Every change re-runs the **full** `npm test` suite (not just the new tests) and must be green before merge. Every new feature or flow change is added to `.claude/sources/test-scenarios.md` — the canonical use-case → scenario registry (happy / failure / edge / automated coverage) — and ships with an automated test covering its high-risk use cases (payment, auth, webhook, ledger). "Implemented" means "tested **and** in the registry." The `qa` and `engineer` agents own this list and act on it without being asked.
